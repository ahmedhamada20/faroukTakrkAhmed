    ...
    <div class="row">
        <div class="col">
            <div class="card card-statistics h-100">
                <div class="card-body">
                    <form action="{{route('silder.update','test')}}" method="post" autocomplete="off"
    ...
    <script>
        $(function () {
            $("#image_updload").fileinput({
                ...
                initialPreview: [
                    @if($data->photo)
                        "{{asset('admin/pictures/silder/' . $data->id . '/'  . $data->photo->Filename)}}"
                    @endif
                ],
                ...
                initialPreviewConfig: [
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('slider_remove_image',['data_id' => $data->id,'photo_id' => $data->photo->id ,'photo_name' => $data->photo->Filename, '_token' => csrf_token()])}}",
                        key: {{$data->photo->id}}
                    }
                ]
                @endif
