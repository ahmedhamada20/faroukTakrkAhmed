    ...
    <div class="row">
        <div class="col">
            <div class="card card-statistics h-100">
                <div class="card-body">
                    <form action="{{route('agencies.update','test')}}" method="post" autocomplete="off"
            ...
            $("#image_updload").fileinput({
                ...
                initialPreview: [
                    @if($data->photos)
                        ...
                        "{{asset('admin/pictures/agencies/' . $data->id . '/'  . $row->Filename)}}",
                    @endforeach
                ...
                initialPreviewConfig: [
                        ...
                        @foreach($data->photos as $row)
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('agencies_photo_remove_image',['data_id' => $data->id,'photo_id' => $row->id ,'photo_name' => $row->Filename, '_token' => csrf_token()])}}",
                        key: {{$row->id}}
                    },
                    @endforeach
                    @endif
    ...
    <script>
        $(function () {
            $("#image_updload_bdf").fileinput({
                ...
                initialPreview: [
                    @if($data->bdf)
                        "{{asset('admin/bdf/agencies/' . $data->id . '/'  . $data->bdf->Filename)}}"
                    @endif
                ],
                initialPreviewAsData: true,
                ...
                initialPreviewConfig: [
                    {
                        ...
                        size: '111',
                        width: "120px",
                        url: "{{route('agencies_remove_image',['data_id' => $data->id,'photo_id' => $data->bdf->id ,'photo_name' => $data->bdf->Filename, '_token' => csrf_token()])}}",
                        key: {{$data->bdf->id}}
                    }
                ]
                @endif
